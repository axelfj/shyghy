...
(function () {
		...
		if (plugins.preloader.length && !isNoviBuilder) {
			pageTransition({
				...
				conditions: function (event, link) {
					return !/(\#|callto:|tel:|javascript:|mailto:|:\/\/)/.test(link) && !event.currentTarget.hasAttribute('data-lightgallery');
				},
	...
	$(function () {
		...
		/**
		 * @desc Forces slider to be horizontal on window width of 576 pixels.
		 * @param {object} swiper - swiper instance
		 */
		function swiperResizeHandler( swiper ) {
			if( window.innerWidth < 576 ) {
				swiper.changeDirection( 'horizontal' );
			} else {
				swiper.changeDirection( swiper.$el[0].getAttribute( 'data-direction' ) || 'horizontal' );
			}
		}
